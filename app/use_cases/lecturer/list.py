...
class ListLecturersRequestObject(request_object.ValidRequestObject):
    ...
    def builder(cls, page_index: int, page_size: int, search: Optional[str] = None,
        ...
        return ListLecturersRequestObject(page_index=page_index,
...
class ListLecturersUseCase(use_case.UseCase):
    ...
    def process_request(self, req_object: ListLecturersRequestObject):
        ...
        total = self.lecturer_repository.count_list(
            match_pipeline=match_pipeline
        )
        ...
        return ManyLecturersInResponse(
            ...
            data=[Lecturer(**LecturerInDB.model_validate(doc).model_dump())
                  for doc in lecturers],
        )

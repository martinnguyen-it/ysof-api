...
from app.infra.tasks.email import send_student_evaluation_subject_task


class SubjectSendEvaluationRequestObject(request_object.ValidRequestObject):
...
class SubjectSendEvaluationUseCase(use_case.UseCase):
    ...
    def process_request(self, req_object: SubjectSendEvaluationRequestObject):
        ...
        self.background_tasks.add_task(
            ...
            AuditLogInDB(
                ...
                description=json.dumps(
                    {
                        "name": "Send email notification evaluation subject",
        ...
        if res:
            send_student_evaluation_subject_task.delay(subject_id=req_object.subject_id)
            return True
        else:

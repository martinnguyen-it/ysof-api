...
class UpdateGeneralTaskRequestObject(request_object.ValidRequestObject):
    def __init__(self, id: str, current_admin: AdminModel, obj_in: GeneralTaskInUpdate) -> None:
        self.id = id
        self.obj_in = obj_in
    ...
    @classmethod
    def builder(
        cls, id: str, current_admin: AdminModel, payload: Optional[GeneralTaskInUpdate] = None
...
class UpdateGeneralTaskUseCase(use_case.UseCase):
    ...
    def process_request(self, req_object: UpdateGeneralTaskRequestObject):
        ...
        self.general_task_repository.update(
            id=general_task.id, data=GeneralTaskInUpdateTime(**req_object.obj_in.model_dump())
        )
        general_task.reload()

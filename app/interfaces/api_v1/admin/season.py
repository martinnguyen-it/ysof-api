...
@response_decorator()
def create_season(
    payload: SeasonInCreate = Body(..., title="Season In Create payload"),
    create_season_use_case: CreateSeasonUseCase = Depends(CreateSeasonUseCase),
...
def get_current_season(
    get_season_use_case: GetCurrentSeasonCase = Depends(GetCurrentSeasonCase),
):
...
def get_season_by_id(
    ...
    get_season_request_object = GetSeasonRequestObject.builder(season_id=season_id)
    response = get_season_use_case.execute(request_object=get_season_request_object)
    return response
...
def update_season(
    ...
    payload: SeasonInUpdate = Body(..., title="Season updated payload"),
    update_season_use_case: UpdateSeasonUseCase = Depends(UpdateSeasonUseCase),
    current_admin: AdminModel = Depends(get_current_active_admin),
):
    authorization(current_admin, [AdminRole.ADMIN])
...
def mark_season_current(
    id: str = Path(..., title="Season Id"),
    mark_current_season_use_case: MarkCurrentSeasonUseCase = Depends(MarkCurrentSeasonUseCase),
    current_admin: AdminModel = Depends(get_current_active_admin),
):
    authorization(current_admin, [AdminRole.ADMIN])
...
def delete_season(
    id: str = Path(..., title="Season Id"),
    delete_season_use_case: DeleteSeasonUseCase = Depends(DeleteSeasonUseCase),
    current_admin: AdminModel = Depends(get_current_active_admin),
):
    authorization(current_admin, [AdminRole.ADMIN])

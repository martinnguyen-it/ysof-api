...
from app.domain.subject.subject_evaluation.entity import ManySubjectEvaluationAdminInResponse, SubjectEvaluationAdmin
...
from app.use_cases.subject_evaluation.list import ListSubjectEvaluationRequestObject, ListSubjectEvaluationUseCase
from app.domain.shared.enum import Sort

router = APIRouter()


@router.get("/detail", dependencies=[Depends(get_current_active_admin)], response_model=SubjectEvaluationAdmin)
@response_decorator()
def get_subject_evaluation(
    ...
    req_object = GetSubjectEvaluationRequestObject.builder(subject_id=subject_id, current_student=student_id)
    response = get_subject_evaluation_use_case.execute(request_object=req_object)
    return response


@router.get("/{student_id}", dependencies=[Depends(get_current_active_admin)], response_model=SubjectEvaluationAdmin)
@response_decorator()
...
@router.get("", dependencies=[Depends(get_current_active_admin)], response_model=ManySubjectEvaluationAdminInResponse)
@response_decorator()
def get_all_subject_evaluation(
    ...
    req_object = ListSubjectEvaluationRequestObject.builder(
        page_index=page_index, page_size=page_size, search=search, sort=sort_query, subject_id=subject_id
    )
